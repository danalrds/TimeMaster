	...
	if ($nr>0)//daca exista cel putin o stire in tabela
	 {
		 ...
		 $query1 = "SELECT * FROM utilizatori LIMIT $start,$rpp";
		 $rezultat=mysql_query($query1);
		 if ($succes=mysql_num_rows($rezultat)>0) 
			{			  
		        ...
		        $str =$rand[1];				
				?>
				<form method="post" action="stergeuser.php">
					<input  type="hidden" name="nume" value="<?php echo $rand[1]; ?>">
				...
				//$str = preg_replace("/([a-z]+)/e","strtoupper('<small>\\1</small>')",$str);
		          ...
		          echo "<tr>";
							echo "<td align='left'>Parola: " .$rand[2]. "</td>"; 
							echo "<td align='center'>Email: " .$rand[5]. "</td>"; 
							echo "<td align='right'>Reducere: ".$rand[6]. "</td>";								
				echo"</tr>";
							?>
							</table> </center>
							<hr color ="#75461C" size="1px" style="width: 800px;"/> 
						     <p style="text-align: right; padding-right:130px; padding-top:0px; margin-top: -5px; margin-bottom: -10px;"> 
	 ...
	 $nav='';//in nav creez  sirul ce contine numerotarea din partea de jos a paginii
			 ...
			 {
			   if ($pagina!=$i)
			     $nav .='<a style="color:#5E3400; text-decoration:none" href="useri.php?pg='.$i.'">'.$i.'</a>|';
			   else
